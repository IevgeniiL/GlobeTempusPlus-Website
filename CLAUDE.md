...
**Images** (`/images/screens/`): app screenshots with fixed names, `01-main.png` … `07-day-agenda.png`. They are generated by the iOS repo (`GlobeTempus-Plus-iOS`) with `scripts/publish-website-screenshots.sh`, which runs its `ScreenshotTests` UI test, resizes the PNGs to 750px wide and opens a pull request here for review (`--publish` pushes straight to main). Replace the files, never rename them — `index.html` and `sw.js` reference these exact paths.
